...
async def sync_live_scores(db, recalculate_match_points, propagate_knockout_winner) -> dict:
    ...
    for fixture in fixtures:
        ...
        # Chercher le match qu'il soit déjà "finished" ou non.
        # Un match peut avoir été marqué "finished" avec un mauvais score
        # (ex : 0-0 au coup d'envoi avant que l'API remonte le vrai résultat).
        match = await db.matches.find_one({
            "home_team": home_fr,
            "away_team": away_fr,
        })
        ...
        already_finished = match.get("status") == "finished"
        score_changed = (
            match.get("home_score_actual") != home_goals
            or match.get("away_score_actual") != away_goals
        )
        ...
        # Si le match est déjà finished ET que le score n'a pas changé,
        # ne pas recalculer inutilement.
        if already_finished and not score_changed:
            continue
        ...
        updated.append({
            ...
            "score": f"{home_goals}-{away_goals}",
            "status": new_status,
            "score_corrected": already_finished and score_changed,
        })

    return {"checked": len(fixtures), "updated": updated}
>>>>>>> d78b3dc (feat: prono special finalistes + vainqueur CdM)
